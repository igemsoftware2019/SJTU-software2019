...
def out_put_file(out_file,name,PURPOSE,DP_LAB,LAB_link,pub,pub_link,seq_link,BB_name,BB_manu,BB_size,Md2BB,Vector_type,Prom,Ba_Re,Gro_Tem,Gro_str,Co_nu,marker,tag,clone,resouce,term):
    out_f=open(out_file,"a")
    out_f.write(out_file+",,"+name+",,"+PURPOSE+",,"+DP_LAB+",,"+LAB_link+",,"+pub+",,"+pub_link+",,"+seq_link+",,"+BB_name+",,"+BB_manu+",,"+BB_size+",,"+Md2BB+",,"+Vector_type+",,"+Prom+",,"+Ba_Re+",,"+Gro_Tem+",,"+Gro_str+",,"+Co_nu+",,"+marker+",,"+tag+",,"+clone+",,"+resouce+",,"+term+"\n")

link="https://www.addgene.org/86209/"
...
LAB_link="https://www.addgene.org"+re.split("\n|</a>|\"","".join('%s' %id for id in (DP_LAB_re.findall(res.text))))[7]
...
seq_link=bs.find(attrs={'class':'sequence_information'})
seq_re=re.compile(r"/\d*/sequences/")
seq_link="https://www.addgene.org"+seq_re.findall(res.text)[1]
use_re=re.compile(r"Vector\stype</div>[\s\S]*?</li>")
use=re.split("<|>|\n","".join('%s' %id for id in (use_re.findall(res.text))))[3]
