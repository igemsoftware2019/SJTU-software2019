...
out_f=open("species_ecnum.sql","a")
...
def show_ecnumber(reaction,file):
    temp_path=file_path+"/"+file
    ...
    for  reaction_info in reaction_list:
        name="UNIQUE-ID - "+reaction+",,"

        if name in reaction_info:

            reaction_info_list=reaction_info.split(",,")

            for signle_reaction_info in reaction_info_list:
                if "EC-NUMBER" in signle_reaction_info:

                    out=signle_reaction_info.replace("EC-NUMBER - ","")
                    break
