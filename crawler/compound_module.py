...
output_file_module="output_module.csv"

#define a list to save the website we have visited
web_list=[]

#define a output file to save the data
out_file_module=open(output_file_module,"w")
out_file_module.write("Entry,,Module,,Module Name,,Link\n")
out_file_module.close()

files=os.listdir(file_path)

#define the function that can save the information
def out_put_file(out_file,entry,module,module_name,link):
    out_f=open(out_file,"a")
    link="https://www.kegg.jp"+link
    out_f.write(entry+",,"+module+",,"+module_name+",,"+link+"\n")
            ...
            if line.split(",")[2]=="compound":
                ...
                if line_link in web_list:
                    ...
                    while temp_line != '':
                        ...
                        elif re_module.search(temp_line):
                            print("1")
                            temp_line=te_f.readline()
                            module=re.compile(r'\>[A-Za-z0-9]{1,}\<\/a\>')
                            module_name=re.compile(r'\<td\>[A-Za-z0-9\s\_\,\/\-\(\)\&\;\=\+]{1,}\<\/td\>')
                            link=re.compile(r'\<a\shref\=\"[0-9A-Za-z\/\-\_\?\+\,\=\>\<]{1,}\"\>')
                            print(module_name.findall(temp_line))
                            n=0
                            for output in module.findall(temp_line):
                                output=output.replace(">","")
                                output_module=output.replace("</a","")
                                output=module_name.findall(temp_line)[n]
                                output=output.replace("</td>","")
                                output=output.replace("&gt;",">")
                                output_module_name=output.replace("<td>","")
                                output=link.findall(temp_line)[n]
                                output=output.replace("<a href=\"","")
                                output_link=output.replace("\">","")
                                n+=1
                                out_put_file(output_file_module,entry_out,output_module,output_module_name,output_link)
